...
class GoogleMap {
    ...
    addMarker(position) {
        ...
        this.marker = new google.maps.Marker({ animation: google.maps.Animation.BOUNCE, position, map: this.map });
    }
}
...
class WeatherServer {
    ...
    async loadMeteo(point, units, language) {
        const url = `http://api.openweathermap.org/data/2.5/weather?lat=${point.lat}&lon=${point.lng}&units=${units}&lang=${language}&APPID=${this.apiKey}`;
        return await fetch(url).then(response => response.json());
    }
}

class Controller {
    constructor(weatherServer, point, units, language, map, parentElement) {
        this.point = point;
        this.units = units;
        ...
        this.map = map;
        this.map.map.addListener('click', (event) => {
            const lat = event.latLng.lat();
            const lng = event.latLng.lng();
            const newPoint = { lat, lng };
            this.point = newPoint;
            this.loadSectionsInView();
            return this.map.addMarker(newPoint);
        });
    }
    async loadSectionsFromServer() {
        return await this.weatherServer.loadMeteo(this.point, this.units, this.language)
            .then(placeMeteo => {
    ...
    loadSectionsInView() {
        ...
        this.loadSectionsFromServer().then(section => this.parentElement.append(section));
    }
    section(place) {
...
initializer.do = function () {
    navigator.geolocation.getCurrentPosition(position => {
        ...
        const weatherServer = new WeatherServer(apikey);
        const controller = new Controller(weatherServer, pG, 'metric', 'es', map, mainSection);
